...
try:
    os.mkdir('/Users/astro502/Chemists/output')
except:
...
# Number star that has been looked at.
b = 0

for j in range(len(host_name_list)):
    star = host_name_list[j + b]
    ...
    output = f"/Users/astro502/Chemists/output/{correct_name}.tbl"
    Koa.query_object ('hires', \
                  star, \
                  output, overwrite=True,)
    
    output_dir = f"/Users/astro502/Chemists/dnload_dir_hires_calib1/{correct_name}"
                    ...
                    for k in range(len(wavelength_full)):
                       if wavelength_full[k] > min_limit[f] and wavelength_full[k] < max_limit[f]:
                          ...
                          # # Fixes the path for the flux.tbl.gz file
                            ...
                            destination_path = f'/Users/Astro502/Chemists/{correct_name}' 
    ...
    print(correct_name)
    try:
        shutil.rmtree(f"/Users/astro502/Chemists/dnload_dir_hires_calib1/{correct_name}")
    except Exception as e:
    ...
    print('Just completed star j =', j+b)
